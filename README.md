...
- 🤖 AI / 🛡️ Security 双分类，AI 10 个 + 安全 7 个免费源（见 `feeds.json`）
...
**Security：** 安全客 · 先知社区 · 嘶吼 RoarTalk · 安全脉搏 · 火绒安全 · 天融信阿尔法实验室 · 吾爱破解病毒分析

> 说明：中文安全类站点多数已停更或下线 RSS，且部分站点（如 FreeBuf）对境外数据中心 IP 封锁，无法从 Vercel/Render 抓取，因此这里只收录境内部署也能稳定拿到的源；个别源更新较慢或偶发超时，代码会自动跳过并提示，不影响其它源。所有源均免费。
