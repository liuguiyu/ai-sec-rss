...
- 🤖 AI / 🛡️ Security 双分类，AI 10 个 + 安全 9 个免费源（见 `feeds.json`）
- 跨源去重、按发布时间倒序，显示来源徽章 + 相对时间 + 摘要；**只保留最近 7 天**内容（环境变量 `MAX_AGE_DAYS` 可调）
- 🔴 漏洞条目自动补 **CVSS 等级**（严重 / 高危 / 中危 / 低危 + 分数）和 **在野利用**标记（CISA KEV）
...
- 后台每 5 分钟静默刷新：**不打断阅读**，有新内容时顶部出现「↑ 有 N 条新内容」提示，点一下才更新
...
**Security：** CISA 在野利用漏洞 · Exploit-DB · 微软安全公告 · Ubuntu 安全公告 · Debian 安全公告 · Tenable 漏洞研究 · 安全客 · 先知社区 · 吾爱破解病毒分析

> 漏洞等级说明：CVSS 等级来自 NVD「最近 CVE」数据（缓存 6 小时），「在野利用」标记来自 CISA KEV 目录（缓存 6 小时）；查不到等级时只显示 CVE 编号，不影响其它内容。
>
> 说明（安全分类）：中文漏洞库（CNVD / CNNVD / 阿里云 AVD / 奇安信 / 微步 / Seebug）目前均不提供可用 RSS，因此漏洞与 CVE 情报采用国际权威源：CISA 在野利用漏洞目录、Exploit-DB、微软 MSRC、Ubuntu / Debian 安全公告、Tenable 研究；中文部分保留仍在持续更新的安全客、先知社区、吾爱破解病毒分析区、嘶吼与天融信阿尔法实验室。
