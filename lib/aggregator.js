...
// 只保留最近 N 天的内容（默认 7 天），可用 MAX_AGE_DAYS 调整
const MAX_AGE_DAYS = Number(process.env.MAX_AGE_DAYS) > 0 ? Number(process.env.MAX_AGE_DAYS) : 7;
...
async function fetchCategoryNow(cat) {
  ...
  // 只要最近 MAX_AGE_DAYS 天的内容（无日期的条目无法判定，保留）
  const cutoff = Date.now() - MAX_AGE_DAYS * 24 * 60 * 60 * 1000;
  const recent = allItems.filter((it) => !it.date || it.date.getTime() >= cutoff);

  // 补 CVE 等级 / 在野利用标记；拿不到就跳过，不影响返回
  await enrich(recent, Number(process.env.ENRICH_BUDGET_MS) || 2500).catch(() => {});

  // 分组计数（安全分类的子标签页用）
  const groups = {};
  for (const it of recent) {
    if (it.group) groups[it.group] = (groups[it.group] || 0) + 1;
  }

  // 总榜取前 ITEM_LIMIT，另外每个分组也各取前 ITEM_LIMIT，
  // 这样安全分类切到子标签页时不会只剩几条（前端按 group 过滤）
  const picked = [];
  ...
  take(recent.slice(0, ITEM_LIMIT));
  for (const g of Object.keys(groups)) {
    take(recent.filter((it) => it.group === g).slice(0, ITEM_LIMIT));
  }
  picked.sort((a, b) => {
    const ta = a.date ? a.date.getTime() : 0;
    const tb = b.date ? b.date.getTime() : 0;
